...
class SWME1D(PDE):
    ...
    def compute_system_matrix(self,
        ...
        if order == 2:
            ...
            A[2][1] = 2*alpha1
            A[2][2] = um + alpha2 #instead of 4*alpha2/5
            A[2][3] = 3*alpha1/5
    ...
    def get_initial_values(self,
        ...
        if initial_condition == 'constantHeight_noVelocity':
            ...
            initial_values[0] = 1 + np.exp(3*np.cos(np.pi*(position+0.5))-4.0)
            initial_values[1] = 0.25*initial_values[0]
            if order > 0:
            ...
            if order > 5:
                initial_values[7] = 0
        elif initial_condition == 'smooth_waveII':
            initial_values[0] = 1.0 - 0.1 *np.power( np.sin(np.pi*position*0.5),2)
            initial_values[1] = 0.25*initial_values[0]
            if order > 0:
                initial_values[2] = -0.25*initial_values[0] 
            if order > 1:
            ...
            if order > 5:
                initial_values[7] = 0  
        elif initial_condition == 'smooth_waveIII':
            initial_values[0] = 1 + np.exp(3*np.cos(np.pi*(position+0.5))-4.0)
            initial_values[1] = 0.25*initial_values[0]
            if order > 0:
                initial_values[2] = -0.25*initial_values[0] 
            ...
            if  position < x0:
                ...
                if order > 5:
                    initial_values[7] = 0 
        elif initial_condition == 'Velocity_squared':
            #initial_values[0] = 1.0+0.1*np.exp(3.0*np.cos(np.pi*(position+0.5))-4.0)
            initial_values[0] = 1.0 - 0.1 *np.power(np.sin(np.pi*position*0.5),2)
            initial_values[1] = initial_values[0]
            if order > 0:
                initial_values[2] = -(0.6)*initial_values[0] 
            if order > 1:
                initial_values[3] = -(1/7)*initial_values[0]
            if order > 2:
                initial_values[4] = -(1.0/15.0)*initial_values[0] 
            if order > 3:
                initial_values[5] = -(3.0/77.0)*initial_values[0]
            if order > 4:
                initial_values[6] = -(1.0/39.0)*initial_values[0]
            if order > 5:
                initial_values[7] = -(1.0/55.0)*initial_values[0]
            if order > 6:
                initial_values[8] = -(3.0/221.0)*initial_values[0]
            if order > 7:
                initial_values[8] = -(1.0/95.0)*initial_values[0]
        elif initial_condition == 'Gaussian':
            initial_values[0] =  1.0-0.1*(np.sin(np.pi*(position)))
            initial_values[1] = 0.25*initial_values[0]
            if order > 0:
                initial_values[2] = -0.25*initial_values[0] 
            if order > 1:
    ...
    def compute_vertical_velocity_profile(self,
        ...
        if order >= 7:
            for i in range(len(values)):
                velocity_profile[i,:] += values[i,9]*(np.ones(len(z_points)) - 56*z_points + 756*np.square(z_points) - \
                                                     4200*np.power(z_points,3) + 11550*np.power(z_points,4) - \
                                                        16632*np.power(z_points,5) + 12012*np.power(z_points,6)- \
                                                        3432*np.power(z_points,7)) 
        if order >= 8:
            for i in range(len(values)):
                velocity_profile[i,:] += values[i,10]*(np.ones(len(z_points)) - 72*z_points + 1260*np.square(z_points) - \
                                                      9240*np.power(z_points,3) + 34650*np.power(z_points,4) - \
                                                        72072*np.power(z_points,5) + 84084*np.power(z_points,6) - \
                                                        51480*np.power(z_points,7)+12870*np.power(z_points,8))   

        if order >= 9:
            for i in range(len(values)):
                velocity_profile[i,:] += values[i,11]*(np.ones(len(z_points)) - 90*z_points + 1980*np.square(z_points) - \
                                                      18480*np.power(z_points,3) + 90090*np.power(z_points,4) - \
                                                        252252*np.power(z_points,5) + 420420*np.power(z_points,6) - \
                                                        411840*np.power(z_points,7)+218790*np.power(z_points,8) - \
                                                        48620*np.power(z_points,9))       
        if order >= 10:
            for i in range(len(values)):
                velocity_profile[i,:] += values[i,12]*(np.ones(len(z_points)) - 110*z_points + 2970*np.square(z_points) - \
                                                      34320*np.power(z_points,3) + 210210*np.power(z_points,4) - \
                                                        756756*np.power(z_points,5) + 1681680*np.power(z_points,6) - \
                                                        2333760*np.power(z_points,7)+1969110*np.power(z_points,8) - \
                                                        923780*np.power(z_points,9)+184756*np.power(z_points,10))
        
        if order >= 11:
            for i in range(len(values)):
                velocity_profile[i,:] += values[i,12]*(np.ones(len(z_points)) - 132*z_points + 4290*np.square(z_points) - \
                                                      60060*np.power(z_points,3) + 450450*np.power(z_points,4) - \
                                                        2018016*np.power(z_points,5) + 5717712*np.power(z_points,6) - \
                                                        10501920*np.power(z_points,7)+12471030*np.power(z_points,8) - \
                                                        9237800*np.power(z_points,9)+3879876*np.power(z_points,10) - 705432*np.power(z_points,11))
        
        if order >= 12:
            for i in range(len(values)):
                velocity_profile[i,:] += values[i,12]*(np.ones(len(z_points)) - 156*z_points + 6006*np.square(z_points) - \
                                                      100100*np.power(z_points,3) + 900900*np.power(z_points,4) - \
                                                        4900896*np.power(z_points,5) + 17153136*np.power(z_points,6) - \
                                                        39907296*np.power(z_points,7) + 62355150*np.power(z_points,8) - \
                                                        64664600*np.power(z_points,9)+ 42678636*np.power(z_points,10) - 16224936*np.power(z_points,11)+2704156*np.power(z_points,12))
...
class Reduced_SWME1D(PDE):
    """
    This class represents the one-dimensional Reduced Shallow Water Moment Equations (SWME1D).
    ...
    def __init__(self, 
                initial_condition: str,
                viscosity: float,
                slip_length: float, hyperbolic: bool):
        """
        Constructs all the necessary attributes for the Reduced SWME1D object.
    ...
    def compute_system_matrix(self, order:int, values:np.array, g = 1) -> np.array:
        A=np.zeros((2,2)) 
        h = values[0]
        um = values[1]/values[0]
        A[0][0] = 0.0
        ...
        if order == 1: 
            ...
            factor2 += 1.0/48.0
            if self.hyperbolic:
                factor3 += 4.0/(192.0*192.0)
        if order > 1:
            factor1 += 1.0/45.0
            factor2 += 1.0/45.0
            if self.hyperbolic:
                factor3 += 4.0/(180.0*180.0)
        
        A[1][0] = g*h-factor2*g*h**3/(self.slip_length**2) - um*um*((1.0- (factor1*h*h/(self.slip_length**2))))+factor3*g*np.power(h,5)/(np.power(self.slip_length,4))
        A[1][1] = 2.*um*(1.0+ factor1*h*h/(self.slip_length**2))
        return A
    ...
    def compute_source_term(self,
        ...
        S[1] = -(self.viscosity/self.slip_length)*um*(1-(factor1*h/(self.slip_length))+(factor2*h*h/(self.slip_length*self.slip_length)))
        return S
    ...
    def get_initial_values(self,
        ...
        if initial_condition == 'constantHeight_noVelocity':
            ...
            initial_values[0] = 1.0 + 0.1*position
            initial_values[1] = 0.0*initial_values[0]
        elif initial_condition == 'smooth_wave':
            initial_values[0] = 3.0 + np.exp(-1.5*position**2)
            initial_values[1] = 0.0*initial_values[0] 
        elif initial_condition == 'smooth_waveII':
            initial_values[0] = 1.0 - 0.1 *np.power( np.sin(np.pi*position*0.5),2)
            initial_values[1] = 0.25*initial_values[0] 
