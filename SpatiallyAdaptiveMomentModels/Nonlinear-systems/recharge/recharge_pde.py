...
from recharge.source_terms import compute_recharge_source

class RechargeSWME1D(SWME1D):
    """
    Minimal recharge-equipped SWME model.

    First implementation:
    - It inherits the SWME transport structure,
    - It replaces the source term with S_{R,I} (pure recharge source, no friction)
    ...
    def compute_source_term(
        ...
        source, _ = compute_recharge_source(
            order = order,
            values = values,
