...
if pde_type_A == 'SWME1D':
    ...
    _pde_A = PDE.VegetationSWME1D(IC_A, viscosity_A, slip_length_A, False, 1, 1, 1)
...
if pde_type_B == 'SWME1D':
    ...
    _pde_B = PDE.SWME1D(IC_B, viscosity_B, slip_length_B, hyperbolic=True)
elif pde_type_B == 'VegetationSWME1D':
    _pde_B = PDE.VegetationSWME1D(IC_B, viscosity_B, slip_length_B, False, 1, 1, 1)
...
elif pde_type_B == 'SGSWME1D':
    print("pde_type can only be SGSWME1D if stochastic_Galerkin is True and spatially_adaptive and monte_carlo are False")
else:
    print('This pde_type is not implemented yet')

if pde_type_C == 'SWME1D':
    _pde_C = PDE.SWME1D(IC_C, viscosity_C, slip_length_C, hyperbolic=False)
elif pde_type_C == 'HSWME1D':
    _pde_C = PDE.SWME1D(IC_C, viscosity_C, slip_length_C, hyperbolic=True)
elif pde_type_C == 'VegetationSWME1D':
    _pde_C = PDE.VegetationSWME1D(IC_C, viscosity_C, slip_length_C, False, 1, 1, 1)
